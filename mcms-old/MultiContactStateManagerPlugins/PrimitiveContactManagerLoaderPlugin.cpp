...
#include <MotionGenerator/MultiContactStateManager/MultiContactStatePrimitives/MCMS/PrimitiveContactManagerLoader.h>
...
TaskExecutionHandler* create_task(TaskExecutionManager* parentManager,
				  const char* name = "mcs-contact-manager-mcms",
				  std::list<std::string>* cmd_init = NULL)
{
  return new PrimitiveContactManagerLoader(parentManager, name, cmd_init);
}
